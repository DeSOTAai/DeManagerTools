...
set manager_path_dev=%UserProfile%\Desota\DeManagerTools
@REM - Model Execs
...
%UserProfile%\Desota\DeManagerTools\dist\DeSOTA - Manager Tools.exe
...
IF NOT EXIST %manager_path_install% GOTO endofreinstall
...
:noreinstallargs
call %manager_uninstall%
GOTO endofreinstall
:reinstall
call %manager_uninstall% /Q
:endofreinstall
IF EXIST %manager_path_install% echo Re-Instalation Required && GOTO EOF_IN
...
@REM Create App EXE
call pyinstaller --onefile -w app.py
call pyinstaller -w -F --uac-admin -i "Assets/icon.ico" -n "DeSOTA - Manager Tools" app.py
