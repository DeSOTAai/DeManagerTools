...
IF EXIST %manager_path_run%\status.txt (
	set /p manager_status= < %manager_path_run%\status.txt
    IF %manager_status% EQU 1 (
        ECHO %fail%Re-Instalation Fail - Close ` Desota - Manager Tools.exe ` before attempting to Uninstall!%ansi_end%
        GOTO EOF_MAN_UN
    )
)
...
:: Delete Project Folder
IF EXIST %manager_path_un% (
    rmdir /S %manager_path_un%  > NUL 2>NUL
    GOTO EOF_MAN_UN
)
